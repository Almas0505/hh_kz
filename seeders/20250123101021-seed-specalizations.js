...
const specalizationTypesUpdated = specalizationTypes.map(item =>{
    return {
      name: item["name"],
...
let specalizationsUpdated = [];
specalizationTypes.forEach((item, index) => {
  item["specializations"].forEach(spec => {
    specalizationsUpdated.push({
      name: spec["name"],
      specalizationTypeId: index + 1
    });
  });
});
...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    await queryInterface.bulkInsert('SpecalizationTypes', specalizationTypesUpdated,{});
  await queryInterface.bulkInsert('Specalizations',specalizationsUpdated ,{});
},

  down: async (queryInterface) => {
    await queryInterface.bulkDelete('SpecalizationTypes', null, {});
  },
};
